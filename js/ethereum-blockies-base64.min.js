...
(function() {
  ...
  function createImageData(size, seedData) {
    var width = size;
    var height = size;
    ...
    for (var y = 0; y < height; y++) {
      for (var x = 0; x < dataWidth; x++) {
        var val = Math.floor(rand(seedData) * 2.3);
        data[y * width + x] = val;
  ...
  function renderImage(data, size, scale, palette) {
    var imgWidth = size;
    var imgHeight = size;
  ...
  function makeBlockie(address, opts) {
    ...
    var canvas = renderImage(data, size, scale, palette);
